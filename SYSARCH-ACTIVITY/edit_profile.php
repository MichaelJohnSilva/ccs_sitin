...
.navbar .right a, .navbar .right .dropdown-btn {
    color: white;
    padding: 10px 18px;
    border-radius: 8px;
    transition: all 0.3s ease;
...
.navbar .right a:hover, .navbar .right .dropdown-btn:hover {
    background: rgba(255,255,255,0.1);
    transform: translateY(-2px);
}
    ...
    <div class="right">
        ...
        <div class="dropdown">
            ...
            <div class="dropdown-content">
                ...
                ?>
                    <p class="notif-item <?php echo htmlspecialchars($notif['type']); ?>" onclick="markRead('<?php echo $userIdNumber; ?>', '<?php echo $redirectUrl; ?>')" style="cursor:pointer;">
...
function markRead(idNumber, redirect) {
    fetch('mark_notif_read.php?id_number=' + idNumber)
        .then(res => res.json())
        .then(data => { window.location.href = redirect; });
}
</script>
